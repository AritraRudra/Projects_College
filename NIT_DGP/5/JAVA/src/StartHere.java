...
public class StartHere {
	...
	static final int popuSize = 128;	//(int) Math.pow(2, 20);	//2^31;	// Population size
	...
	static final int tournamentSize = 5;//(popuSize/noOfNodes);	//5
	static final boolean elitism = true;	//save best two across generations

	public static void main(String[] args) {
		int i,j, a = 0, b;
		int[] route;
		double costDelay = 0, costEnrg = 0, dist = 0;		
		//String dataFilesPath = "C://Temporary_1//Matlab//5//";
		String dataFilesPath = "D://College//Project//5//Data//Set_5//";

		/* variables required for graphs */
		//String resultFilePath = "C://Temporary_1//Data//Result//";
		String resultFilePath = "D://College//Project//5//Results//Set_5//";
		double sum; 
		double[] bestFitnessEachGen = new double[generationCount];
		double[] avgFitnessEachGen = new double[generationCount];
		double[] bestDelayEachGen = new double[generationCount];
		double[] bestEnergyEachGen = new double[generationCount];
		double[] avgDelayEachGen = new double[generationCount];
		double[] avgEnergyEachGen = new double[generationCount];
		...
		try {
			...
			String tuningInfFile=new String(resultFilePath + "S_Mod.txt");		// to get the locally modified S values
		...
		for( i = 0; i < generationCount; i++){
			...
			bestDelayEachGen[i]=myPop.getDelayOfFittest();
			bestEnergyEachGen[i]=myPop.getEnergyOfFittest();
			avgDelayEachGen[i]=myPop.getAvgDelayOfPop();
			avgEnergyEachGen[i]=myPop.getAvgEnergyOfPop();

			sum = 0.0;
			for(j = 0; j<2*(popuSize/3);j++)
				sum += myPop.getIndividual(j).getDelayCost();			
			djDeley[i] = (sum/(2*(popuSize/3)));
			sum = 0.0;
			for(j = 0; j<2*(popuSize/3);j++)
				sum += myPop.getIndividual(j).getEnrgCost();
			djEnrg[i] = (sum/(2*(popuSize/3)));
			/*
			djDeley[i] = myPop.getIndividual(0).getDelayCost();
			djEnrg[i] = myPop.getIndividual(0).getEnrgCost();

			/* for percentage  completed */
			dist = (((double)i)/generationCount)*100;
		...
		/*try {
			...
			File fileName;
			fileName = new File(resultFilePath + "Fitness_vs_Gen.txt");
			...
			/*
			fileName = new File(resultFilePath + "QOS_across_Gen.txt");
			writer = new BufferedWriter(new FileWriter(fileName));

			for( i = 0; i < generationCount; i++)
				writer.write(Double.toString(bestDelayEachGen[i]) + " ");			
			writer.write("\n");	
			for( i = 0; i < generationCount; i++)
				writer.write(Double.toString(bestEnergyEachGen[i]) + " ");			
			writer.write("\n");
			for( i = 0; i < generationCount; i++)
				writer.write(Double.toString(avgDelayEachGen[i]) + " ");			
			writer.write("\n");			
			for( i = 0; i < generationCount; i++)
				writer.write(Double.toString(avgEnergyEachGen[i]) + " ");			
			writer.flush();
			...
			*/
			...
			fileName = new File(resultFilePath + "Dijkstra_Comparision.txt");
