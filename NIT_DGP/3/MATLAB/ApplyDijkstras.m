...
trnsCstMat = load('Transmission_Cost_Data.txt');
...
dest = 60; %length(adjMat);
 [costs,paths] = dijkstra(adjMat,trnsCstMat,1,dest)
...
%         if(adjMat(i,j) == 1)
%             tempMat(i,j) = trnsCstMat(i,j) + costEnrgMat(i,j);
