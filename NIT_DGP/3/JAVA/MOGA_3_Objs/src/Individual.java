...
public class Individual {
	...
	private double meanHops, meanTrnsCst,SDHops, SDTrnsCst;
	...
	public void generateChromosome(String keyword, int noOfNodes, int goalNode) {
		...
		if(keyword.equals("RANDOM")){
			...
			//Random rand = new Random();
			//rand = new Random();
			this.chromosomeLength = rand.nextInt((noOfNodes-2)) + 2;	//min length = 2, max length = 32	//TODO
	...
	public void setNetworkProp(int[][] adjMat, double[][] trnsCstMat) {
		this.adjMat = adjMat;
		this.trnsCstMat = trnsCstMat;
	}
	...
	public int getHops(){
		return (this.chromosomeLength-1);
	}
	...
	public void setMeanHops(double meanHops) {
		this.meanHops = meanHops;
	}

	public double getMeanTrnsCst() {
		return meanTrnsCst;
	}

	public void setMeanTrnsCst(double meanTrnsCst) {
		this.meanTrnsCst = meanTrnsCst;
	}

	public double getSDHops() {
		return SDHops;
	}

	public void setSDHops(double sDHops) {
		SDHops = sDHops;
	}

	public double getSDTrnsCst() {
		return SDTrnsCst;
	}

	public void setSDTrnsCst(double sDTrnsCst) {
		SDTrnsCst = sDTrnsCst;
	}

	public double getTrnsCst() {
		if (this.trnsCst == 0.0) {
			this.trnsCst = this.calculateTrnsCst();
			return (this.trnsCst);
		}else 
			return (this.trnsCst);
	}
	...
	public double getTrnsCst(int[][] adjMat, double[][] trnsCstMat) {
		this.setNetworkProp(adjMat, trnsCstMat);
		if (this.trnsCst == 0.0) {
			this.trnsCst = this.calculateTrnsCst();
			return (this.trnsCst);
		}else 
	...
	//Route trnsCst calculation	
	private double calculateTrnsCst() {
		int a, b, i, len = this.chromosomeLength -1;
		double pathTrnsCst = 0.0;
		for( i = 0; i < len ; i++){
			a = this.genes[i];
			b = this.genes[i+1];
			//if(this.adjMat[a][b] == 1)		//When there is exists an edge between a and b
				pathTrnsCst += this.trnsCstMat[a][b];
			/*else	//When there is no edge between a and b
				pathtrnsCst = Double.MAX_VALUE;*/
		}
		//System.out.println("trnsCst " + pathtrnsCst);
		return pathTrnsCst;
	}
